...
The reporting window is limited to 366 days, future-dated reporting beyond the next day is rejected, and every successful report access emits a `margin_report.viewed` audit event containing only the requested period. This constrains accidental bulk extraction while keeping the report useful for finance and operations.
