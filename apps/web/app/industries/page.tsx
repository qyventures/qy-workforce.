...
export const metadata: Metadata = {
  ...
  description: 'Flexible staffing support for hospitality, F&B, cleaning, retail, promotions and events in Singapore.',
...
export default function Industries() {
  return <main id="main-content" style={{minHeight:'100vh',background:'#F7F8FB',color:'#101828'}}><SiteHeader />
    <script type="application/ld+json" dangerouslySetInnerHTML={{__html: JSON.stringify({
      ...
      '@type': 'CollectionPage',
      name: 'Industries supported by QY Workforce',
      description: 'Flexible staffing support for Singapore operations across six industries.',
      hasPart: industries.map(([name]) => ({ '@type': 'Thing', name })),
    })}} />
    <section style={{maxWidth:1080,margin:'0 auto',padding:'64px 24px'}}>
      ...
      <div style={{marginTop:34,display:'flex',gap:12,flexWrap:'wrap'}}><ConversionLink href="/employers" event="industries_employer_cta" style={{background:'#111827',color:'#fff',padding:'13px 18px',borderRadius:10,textDecoration:'none',fontWeight:800}}>Request manpower</ConversionLink><Link href="/how-it-works" style={{background:'#fff',border:'1px solid #D0D5DD',color:'#344054',padding:'13px 18px',borderRadius:10,textDecoration:'none',fontWeight:800}}>How it works</Link></div>
