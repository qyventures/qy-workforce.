-- Structural/security regression checks for 0029_privacy_request_state_machine.sql.
-- These checks intentionally avoid production credentials and destructive data setup.

begin;

-- Function must remain SECURITY DEFINER with a pinned search_path.
do $$
begin
  if not exists (
    select 1
...
-- Authenticated users may call the RPC, but direct table mutation must remain revoked.
do $$
begin
...
-- Guard against later edits accidentally removing the critical transition/hold controls.
do $$
declare
  v_def text;
begin
