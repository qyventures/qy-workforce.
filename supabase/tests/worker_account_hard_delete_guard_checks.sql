...
begin
  ...
  if position('search_path=pg_catalog,public' in replace(v_fn_search_path, ' ', '')) = 0 then
    raise exception 'prevent_worker_profile_hard_delete must pin search_path=pg_catalog,public';
  end if;
  ...
  if position('worker_profiles' in coalesce(v_fn_source, '')) = 0
     or position('privacy erasure and retention workflow' in coalesce(v_fn_source, '')) = 0 then
    raise exception 'delete guard must protect worker rows and direct erasure through retention controls';
  end if;

  if position('allow_worker_hard_delete' in coalesce(v_fn_source, '')) > 0 then
    raise exception 'worker hard-delete guard must not expose a session-setting bypass';
  end if;
end;
$$;
