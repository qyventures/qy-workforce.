...
-- Finance-only export. Keep bank/payment credentials outside the workforce database.
-- The export contains payroll calculation data only; payment rails can consume it later.
...
begin
  ...
  values (
    auth.uid(), 'payroll_export_recorded', 'payroll_batch', p_batch,
    jsonb_build_object('format', p_format, 'count', p_count, 'checksum', p_checksum)
  );
end;
$$;
...
-- Central retention policy registry so deletion jobs can be implemented consistently.
...
for select using (public.has_app_role(array['admin','auditor']));
create policy retention_admin_write on public.data_retention_policies
for all using (public.has_app_role(array['admin'])) with check (public.has_app_role(array['admin']));
