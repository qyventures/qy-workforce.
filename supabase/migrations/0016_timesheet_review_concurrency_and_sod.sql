...
begin
  ...
  if v_site is null then raise exception 'timesheet not found'; end if;
  if v_status <> 'submitted' then raise exception 'timesheet is no longer awaiting review'; end if;
  ...
  if p_decision='approve' then
    ...
    set status='approved', approved_by=auth.uid(), approved_at=now(), rejected_at=null,
    ...
    update public.timesheets
    set status='rejected', rejected_at=now(), rejection_reason=left(trim(p_rejection_reason),500),
        approved_by=null, approved_at=null, updated_at=now()
    where id=p_timesheet_id and status='submitted';
  end if;

  if not found then raise exception 'timesheet review conflict'; end if;

  insert into public.audit_events(actor_id,action,entity_type,entity_id,metadata)
  values(auth.uid(),'timesheet.' || case when p_decision='approve' then 'approved' else 'rejected' end,
    'timesheet',p_timesheet_id,jsonb_build_object('site_id',v_site,'decision',p_decision));
end;
$$;
