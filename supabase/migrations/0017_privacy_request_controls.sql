...
-- All writes go through audited RPCs. No authenticated direct writes.
...
begin
  ...
  if not exists (select 1 from public.worker_profiles wp where wp.user_id = auth.uid()) then
    raise exception 'worker profile not found';
  end if;

  if exists (
    select 1 from public.privacy_requests pr
    where pr.worker_id = auth.uid()
      and pr.request_type = p_request_type
      and pr.status in ('submitted','in_review','approved')
  ) then
    raise exception 'an active request of this type already exists';
  end if;
  ...
  values (
    auth.uid(), 'privacy_request.submitted', 'privacy_request', v_id,
...
begin
  ...
  if p_decision in ('rejected','cancelled') and nullif(trim(coalesce(p_reason,'')),'') is null then
  ...
  insert into public.audit_events(actor_id, action, entity_type, entity_id, metadata)
  values (
    auth.uid(), 'privacy_request.reviewed', 'privacy_request', p_request,
    jsonb_build_object(
      'request_type', v_type,
      'decision', p_decision,
      'retention_hold', p_retention_hold
