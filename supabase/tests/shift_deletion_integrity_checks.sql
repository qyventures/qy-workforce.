...
-- API roles must not retain destructive DELETE privilege on shifts.
do $$
begin
...
-- A BEFORE DELETE guard must remain installed so later policy/grant changes do
-- not silently reintroduce destructive authenticated deletion.
do $$
begin
  if not exists (
    select 1
...
-- The trigger helper needs no elevated owner privilege: it only inspects the
-- caller identity and rejects destructive API use. Keep search_path pinned and
-- direct PUBLIC execution revoked.
do $$
declare
...
begin
  ...
  if coalesce(v_security_definer, false) then
    raise exception 'prevent_authenticated_shift_delete must not be SECURITY DEFINER';
  end if;
