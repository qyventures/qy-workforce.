...
begin
  if exists (
    ...
    where n.nspname='public' and p.proname in (
      'get_worker_review_queue','review_worker_role','review_worker_vetting','set_worker_operational_status'
    ) and not p.prosecdef
  ...
  if not exists (
    ...
    where n.nspname='public' and p.proname='review_worker_role'
      ...
      and pg_get_functiondef(p.oid) ilike '%worker_role.%'
  ) then raise exception 'role review must lock, prevent self-review and audit'; end if;
