...
Vetting and training evidence is also database-constrained on forward writes. Pending vetting cannot carry reviewer evidence; reviewed outcomes require an Ops reviewer and timestamp, while failed/manual-review outcomes require bounded redacted notes. A passed training module requires completion and verifier evidence, and non-passed rows cannot carry a verifier. Readiness therefore cannot be made deployable by setting a status without the corresponding evidence.
