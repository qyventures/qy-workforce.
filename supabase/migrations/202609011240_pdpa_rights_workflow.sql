...
-- 0017 predates this broader PDPA workflow and already creates
-- privacy_requests with worker_id/retention_hold and a narrower lifecycle.
-- Keep those records and add the newer fields instead of relying on
-- CREATE TABLE IF NOT EXISTS (which does not reconcile an existing schema).
...
alter table public.privacy_requests
  ...
  add column if not exists correction_target text,
...
set requester_id = worker_id
where requester_id is null and worker_id is not null;

do $$
declare v_constraint record;
begin
  -- Constraint names are generated by PostgreSQL in the legacy migration;
  -- identify only the two checks being widened.
  for v_constraint in
    select c.conname
    from pg_constraint c
    where c.conrelid = 'public.privacy_requests'::regclass
      and c.contype = 'c'
      and pg_get_constraintdef(c.oid) ilike '%request_type%'
  loop
    execute format('alter table public.privacy_requests drop constraint %I', v_constraint.conname);
  end loop;
  for v_constraint in
    select c.conname
    from pg_constraint c
...
alter table public.privacy_requests
  ...
  check (request_type in ('access','export','erasure','correction','deletion')),
  add constraint privacy_requests_status_check
  check (status in ('submitted','identity_verified','in_review','approved','partially_approved','rejected','completed','cancelled','withdrawn'));
...
drop policy if exists "workers read own privacy requests" on public.privacy_requests;
